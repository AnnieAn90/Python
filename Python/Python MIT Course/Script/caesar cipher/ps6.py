...
def is_word(word_list, word):
    ...
    '''
    word = word.lower()
    word = word.strip(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"") # strip all punctuations
...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in string.ascii_letters:
            if ord(letter)<91:
                mapping[letter] = chr(((ord(letter)+shift)-65)%26+65)
            else:
                mapping[letter] = chr(((ord(letter)+shift)-97)%26+97)   
        return mapping.copy()
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            ...
            shifted_words = self.apply_shift(25-s)
            text = shifted_words.split(' ')
            
            for letter in text:
                if is_word(wordlist,letter):
                    num += 1
                decrypt_text.append(letter) 

            if num >= num_pre:
                s_pre = 25-s
                text_pre = ' '.join(decrypt_text)
                num_pre = num
                
        return (s_pre,text_pre)  

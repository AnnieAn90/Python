...
# Scale the data (remove mean and divide by std)
def scaleAttrs(vals):
    """ vals is a list """
    vals = pylab.array(vals) # convert to an array
    mean = sum(vals)/len(vals) # get mean
    sd = numpy.std(vals) # std deviation
    vals = vals - mean # remove mean
    return vals/sd # remove mean then devide by std 
...
def trykmeans(examples, numClusters, numTrials, verbose = False):
    ...
    minDissimilarity = cluster.dissimilarity(best) # call the dissimilarity function
    trial = 1
...
# true means scale the data
patients = getData(True) 

for k in (2,4,6):
    print('\n     Test k-means (k = ' + str(k) + ')')
    posFracs = testClustering(patients, k, 2) # call testClustering function


# get number of true positive patients
numPos = 0
for p in patients:
    if p.getLabel() == 1:
        numPos += 1

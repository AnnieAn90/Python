...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    copy = hand.copy() # copy a new dicitonary
    for i in word:
        copy[i] = copy.get(i)-1
    return copy
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while l>0:
        ...
        if word == '.':
            ...
            total = score + total
            print('"%s"' %word," earned "+str(score)+" points. Total:"+str(total)+" points")
            newhand = updateHand(newhand, word)    
            l = calculateHandlen(newhand)
        print()
    if l==0:
